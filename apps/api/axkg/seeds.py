...
PROMPT_SEEDS: list[dict] = [
    ...
    },
    {
        ...
        "prompt_text": (
            ...
            "최종 경로를 조립한다.\n"
            "- **frontmatter 직렬화**: draft_markdown 선두 frontmatter에서 리스트 필드"
            "(`aliases`/`tags`/`up`)는 반드시 inline flow(`[\"별칭1\", \"별칭2\"]`)로 쓰고, 문자열 "
            "값은 큰따옴표로 감싼다. 여러 줄 block list(줄바꿈 후 `- item`)는 **금지** — 문서함 "
            "파서가 읽지 못해 별칭·태그가 통째로 비어 보인다. enum 필드(`type`/`status`)는 템플릿 "
            "표기를 그대로 따른다.\n"
    ...
    },
    {
        ...
        "prompt_text": (
            ...
            "- **frontmatter 직렬화**: 원본요약 선두 frontmatter에서 리스트 필드"
            "(`aliases`/`tags`/`up`)는 반드시 inline flow(`[\"별칭1\", \"별칭2\"]`)로 쓰고, 문자열 "
            "값은 큰따옴표로 감싼다. 여러 줄 block list(줄바꿈 후 `- item`)는 **금지** — 문서함 "
            "파서가 읽지 못해 별칭·태그가 통째로 비어 보인다. enum 필드(`type`/`status`)는 템플릿 "
            "표기를 그대로 따른다.\n"
            "- **filename_candidate는 distinctive하게**: 회사에는 요구 docx가 여러 개 유입되므로 "
            "generic `원본요약`(또는 `{회사}-원본요약`만)은 **금지**한다 — 같은 회사의 다른 요구와 "
            "겹쳐 충돌한다. 이 요구의 핵심 주제/부서를 반영해 `{회사}-<요구주제>-원본요약` 형태로 "
            "정보성 있게 낸다(예: `the-sc-리뷰관리-원본요약`, `the-sc-병원예약-원본요약`).\n\n"
            "## plan = 기능목록(발주서)\n"
    ...
    },
    {
        ...
        "prompt_text": (
            ...
            "- **frontmatter 직렬화**: `markdown_full` 선두 frontmatter에서 리스트 필드"
            "(`aliases`/`tags`/`up`)는 반드시 inline flow(`[\"별칭1\", \"별칭2\"]`)로 쓰고, 문자열 "
            "값은 큰따옴표로 감싼다. 여러 줄 block list(줄바꿈 후 `- item`)는 **금지** — 문서함 "
            "파서가 읽지 못해 별칭·태그가 통째로 비어 보인다. enum 필드(`status`/`priority`)는 "
            "템플릿 표기를 그대로 따른다.\n\n"
            "출력은 output_schema를 따르는 JSON 하나로만 한다(코드펜스·해설 없이 JSON 객체만, "
