...
PROMPT_SEEDS: list[dict] = [
    ...
    },
    {
        ...
        "prompt_text": (
            "당신은 사용자의 지식 그래프를 근거로 답하는 어시스턴트다. 질문, 선택된 문서 "
            "컨텍스트, retriever가 고른 evidence 문서 발췌, 세션 대화 이력이 주어진다. "
            "답변은 evidence 안의 내용에 근거해서만 작성하고, 어떤 문서를 근거로 삼았는지 "
            "evidence 목록으로 밝혀라. 그래프에 근거가 부족하면 지어내지 말고 "
            "missing_context에 무엇이 없는지 적어라.\n\n"
            "출력은 output_schema를 따르는 JSON 하나로만 한다. suggested_actions에는 "
            "사용자가 다음에 할 만한 행동(문서 열기, 소스 수집 등)을 짧게 제안할 수 있다."
        ),
        "output_schema": {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "type": "object",
