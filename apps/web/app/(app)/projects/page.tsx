// 회사 프로젝트 (AXKG-SPEC-014 U-1~U-5) — "프로젝트 추가" + corp 트리 열람(admin 전용).
// 좌: 회사 프로젝트 목록 → 선택 corp 의 origin/baseline/spec 3층 트리(GET /projects/{corp}).
// 우: 선택한 baseline/spec 문서 본문(U-5) — SPEC-013 문서 read-through 재사용(path → GET /documents/{id}).
...
type FolderKind = "origin" | "baseline" | "spec";
const FOLDER_ORDER: FolderKind[] = ["origin", "baseline", "spec"];
...
export default function ProjectsPage() {
  ...
  // --- 본문 로드 (baseline/spec 만; origin 은 비-노드라 본문 없음) ---
  useEffect(() => {
    if (!selected) return;
    const leaf = selected;
    if (leaf.folder === "origin") {
      setBodyMeta(null);
      setBodyError(null);
      setBodyLoading(false);
      return;
    }
    const path = `projects/${leaf.corp}/${leaf.folder}/${leaf.name}`;
    const doc = docIndex.get(path);
    if (!doc) {
      setBodyMeta(null);
      setBodyLoading(false);
  ...
  const selectedKey = selected
    ? `${selected.corp}/${selected.folder}/${selected.name}`
    : null;
      ...
      <div className="mb-4 flex shrink-0 items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            회사별 origin·baseline·spec 문서를 트리로 열람합니다. 프로젝트 추가는 수동·독립 스캐폴딩입니다.
          </p>
        </div>
        <button
          type="button"
...
/** corp 트리의 origin/baseline/spec 3층 폴더 + 항목 렌더(U-3). */
function FolderGroups({
  ...
  const empty = FOLDER_ORDER.every((f) => (tree.folders[f]?.length ?? 0) === 0);
  if (empty) {
    return (
      <p className="px-8 py-1.5 text-xs text-muted-foreground">
        스캐폴드만 있고 문서는 아직 없습니다.
      </p>
    );
  }
  return (
            ...
            {items.length === 0 ? (
              ...
              <ul>
                {[...items]
                  ...
                  .map((name) => {
                    const key = `${tree.corp}/${folder}/${name}`;
                    ...
                    return (
                      <li key={name}>
                        <button
                          type="button"
                          onClick={() => onSelect({ corp: tree.corp, folder, name })}
                          aria-current={active ? "true" : undefined}
                          title={name}
