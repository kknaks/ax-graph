...
## 회사 루트 up: 체인 (회사 앵커로 수렴, AXKG-DEC-009/WORK-013)

회사 프로젝트에는 **회사 루트 앵커 문서 `projects/{corp}/{corp}.md`(document_type `company`, 회사당 1개)** 가 있고, 모든 산출이 `up:` 체인으로 이 루트에 수렴한다. `up:` stem 주입과 본문 `[[{corp}]]` 링크는 **시스템이 corp를 알고 자동 생성**한다(AI가 지어내지 않는다) — 그래도 그래프 엣지의 단일 소스는 본문 `[[ ]]`이므로 `up:`에 넣은 stem은 본문에도 반드시 있다(빈 `[[ ]]` 금지).

- **원본요약(baseline) → 회사 루트**: 원본요약 frontmatter `up: [{corp}]` + 본문 `## 연결`에 `[[{corp}]]`.
- **회사 context(`projects/{corp}/context/`) → 회사 루트**: context 문서(회사 배경지식, 팬아웃 없는 단일 문서)도 `up: [{corp}]` + 본문 `[[{corp}]]`.
- **기능정의서(spec) → 회사 루트(2단 체인)**: spec은 `up: [원본요약 stem]`(자기 baseline). 원본요약이 다시 `up: [{corp}]`이므로 **spec→원본요약→{corp}** 전이 체인으로 회사 루트에 수렴한다(기존 baseline↔spec 관계를 깨지 않고 앵커만 얹음).
- **회사 루트 자체**: 프로젝트 최상위 앵커라 `up:`은 비거나 전역 앵커만 갖는다(회사 루트끼리 위로 묶지 않는다).
- 회사 루트는 그래프 노드(origin과 달리)이므로 `[[{corp}]]`는 index에서 resolve된다 — 회사 프로젝트 산출을 apply하기 전에 회사 루트가 먼저 인덱싱돼 있어야 한다("프로젝트 추가" 시점에 생성·인덱싱).
